...
//        }
    ...
    public ResponseEntity<ApiResponse> processCallBack(HttpHeaders httpHeaders, CallBackResponse callBackResponse)
        ...
        // Validate Headers | no need for headers
//        HeaderErrorMessage headerErrorMessage = validations.validateHeaders(httpHeaders);
//
//        if (!headerErrorMessage.getInvalidHeaderErrors().isEmpty() || headerErrorMessage.isMissingHeaders()) {
//            return new ResponseEntity<>(ApiResponse.responseFormatter(HttpStatus.BAD_REQUEST.value(),
...
//        String sourceSystem = httpHeaders.get(X_SOURCE_SYSTEM).get(0);
        ...
        // this map gets me the specific item in the set i need the Cust ref ID
        Map<String, String> callBackCustRefId =
        ...
        String custRefId = callBackCustRefId.get("ClientTransactionId");
...
//        String callBackCode = callBackResponse.getRequestId();
        ...
        // Log service input
            LogsManager.info(custRefId, TRANSACTION_TYPE, "processCallBackRequest", "",
                    msisdn, "sdp-callback", TARGET_SYSTEM, "", RESPONSE_CODE_0, "",
                    "", "HEADERS: = ".concat(httpHeaders.toString()), "");
            ApiResponse apiResponse = this.callBackResp(custRefId, "sdp-callback", callBackResponse, operation);


        LogsManager.info(callBackResponse.getRequestId(), TRANSACTION_TYPE, "Process Call Back", "",
                msisdn, "callback", TARGET_SYSTEM, "", RESPONSE_CODE_0, operation,
                "", parseToJsonString(callBackResponse), parseToJsonString(apiResponse));

        

        return new ResponseEntity<>(apiResponse, HttpStatus.valueOf(apiResponse.getApiHeaderResponse().getResponseCode()));
    }
    private ApiResponse callBackResp(String requestReferenceID, String sourceSystem, CallBackResponse callBackResponse, String operate)
        ...
        CallBackResponse callBackRequest = prepRequest(callBackResponse,  operate, callBackMsisdn,encryptMsisdn);



           CallBacks url = callBackRepository.findByRequestId(requestReferenceID);

        String restTemplateResponse;
           if (url != null) {
               restTemplateResponse = this.callBackRestUtil.makeAPICall(sourceSystem, "", X_CORRELATION_CONVERSATION_ID,
                       requestReferenceID, url.getUrl(), "", encryptMsisdn, callBackRequest);
           } else {
               restTemplateResponse = "5000";
           }
        ...
        switch (restTemplateResponse){
            ...
            case "5000" :
                responseCode = "400";
                responseBody.setStatus(responseCode);
                responseBody.setDescription("The request id does not exist!!");
                message = "The request Id does not match | exists";
                ...
                break;
            default:
                CallResponse callBackResponse1;
                if (isJSONValid(restTemplateResponse)){
                    callBackResponse1 = objectMapper.readValue(restTemplateResponse, CallResponse.class);
                }
                callBackResponse1 = new CallResponse();

                callBackResponse1.setRequestDesc(restTemplateResponse);
...
//         ErrorMapping errorMapping = this.getResponseMessage(responseCode);
    ...
    private CallBackResponse prepRequest(CallBackResponse apiRequest, String operationName,Map<String, String> callBackMsisdn, String encryptMsisdn){
